...
import EmojiRowList from "./components/EmojiRowList.js";
import EmojiInput from "./components/EmojiInput.js";
import "./App.css";
...
function App() {
  ...
  function filterEmojis(emojis, searchStr) {
    let re = new RegExp(searchStr, "i");
    return emojis.filter((emoji) => re.test(emoji.title));
  }

  return (
    <div className="App">
      <div className="app-header">🤑 Emoji Search 🤑</div>
      <EmojiInput setAppStringState={setSearchStr}></EmojiInput>
      <EmojiRowList emojis={filterEmojis(emojis, searchStr)}></EmojiRowList>
    </div>
  );
}
